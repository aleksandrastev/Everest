...
public class DashboardController implements Initializable {
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		httpMethodBox.getItems().addAll(HTTPConstants.GET, HTTPConstants.POST, HTTPConstants.PUT, HTTPConstants.PATCH,
		...
		bodyTab.disableProperty().bind(Bindings.or(httpMethodBox.valueProperty().isEqualTo(HTTPConstants.GET),
		...
		responseTypeBox.getItems().addAll(HTTPConstants.JSON, HTTPConstants.XML, HTTPConstants.HTML,
		...
		responseTypeBox.valueProperty().addListener(change -> {
			...
			if (type.equals(HTTPConstants.JSON)) {
				responseArea.setText(responseArea.getText(), FormatterFactory.getHighlighter(type),
		...
		try {
			...
			case HTTPConstants.PATCH:
				...
				if (bodyTabController.rawTab.isSelected()) {
					dataRequest.setContentType(
							HTTPConstants.getComplexContentType(bodyTabController.rawInputTypeBox.getValue()));
			...
			errorDetails.setText(
					"Something went wrong. Try to make another request.\nRestart Everest if that doesn't work.");
	...
	private void onFailed(Event event) {
		...
		LoggingService.logWarning(httpMethodBox.getValue() + " request could not be processed.", exception,
				LocalDateTime.now());
	...
	private void prettifyResponseBody(String body, String contentType) {
		...
		try {
			...
			if (contentType != null) {
				/*
				 * Selects only the part preceding the ';', skipping the character encoding. For
						...
						snackbar.show("Open link in browser?", "YES", 5000, e -> {
							...
							new Thread(() -> {
								try {
									...
									LoggingService.logWarning("Invalid URL encountered while opening in browser.", ex,
											LocalDateTime.now());
								}
							}).start();
						});
			...
			responseArea.setText(body, FormatterFactory.getHighlighter(simplifiedContentType),
	...
	private void copyResponseArea() {
		responseArea.getContent();
		String text = responseArea.getContent().getText();
	...
	private void addParamField(String key, String value, boolean checked) {
		/*
		 * Re-uses previous field if it is empty, else loads a new one. A value of null
		 * for the 'event' parameter indicates that the method call came from code and
		 * not from the user. This call is made while recovering the application state.
		 */
		if (paramsControllers.size() > 0) {
			...
			if (previousController.isKeyFieldEmpty() && previousController.isValueFieldEmpty()) {
				previousController.setKeyField(key);
				previousController.setValueField(value);
				previousController.setChecked(checked);

				/*
				 * For when the last field is loaded from setState. This makes sure an extra
				 * blank field is always present.
				 */
	...
	public void setState(DashboardState state) {
		...
		/*
		 * Sanity check to ensure that the state being applied belongs to the active
		 * tab. Everest works perfectly almost every time despite the 4 lines that
		 * follow this comment, but some moronic testers like me might obliterate their
		 * Ctrl + Tab key combo. While switching between tabs at such speeds that
		 * approach that of light, Everest might apply the state to some other tab.
		 * 
		 * This may happen when a RequestManager that was handed over to a
		 * DashboardState were to change its state during a tab shift.
		 */
		...
		if (state.visibleResponseLayer != null) {
			...
			case LOADING:
				/*
				 * Accepts a RequestManager which is in the RUNNING state and switches its
				 * handlers. The handlers affect the Dashboard directly rather than the
				 * DashboardState.
				 */
		...
		/*
		 * The only value from a set of constants in the state.json file is the
		 * httpMethod which, if manipulated to a non-standard value by the user, would
		 * still be loaded into the httpMethodBox, causing severe errors while making
		 * requests.
		 * 
		 * To prevent this, we check if it is a standard value (GET, POST, PUT, PATCH or
		 * DELETE) and only then proceed to applying the rest of the state values to the
		 * Dashboard.
		 * 
		 * This is an extreme case, but still something to be taken care of.
		 */
		boolean validMethod = false;
		String[] httpMethods = { HTTPConstants.GET, HTTPConstants.POST, HTTPConstants.PUT, HTTPConstants.PATCH,
				HTTPConstants.DELETE };
		...
		if (!validMethod) {
			LoggingService.logInfo("Application state file was tampered with. State could not be recovered.",
					LocalDateTime.now());
			return;
		}
		...
		if (!(state.composer.httpMethod.equals(HTTPConstants.GET)
				|| state.composer.httpMethod.equals(HTTPConstants.DELETE)))
